import Image from "next/image";

import SS from "@/assets/images/d.jpg";
import _1 from "@/assets/images/_1.jpg";
import _2 from "@/assets/images/_2.jpg";

import FarmImg from "@/assets/images/farm.jpg";

import { BsArrowUpRightCircle } from "react-icons/bs";
import { HiArrowTrendingUp } from "react-icons/hi2";
import { ImArrowUpRight2 } from "react-icons/im";
import Link from "next/link";
import DisplayLayout from "@/components/layout";


export default function Home() {
    return (
        <div>
            <DisplayLayout>
                <>
                    <div className="px-20 py-32">
                        <div className="w-[70rem] mx-auto">
                            <h3 className="text-5xl font-bold text-center mb-5 w-[70%] mx-auto leading-normal">
                                Improving Farming With Innovative Ideas For The Future.
                            </h3>
                            <div className="h-[30rem] w-full rounded-3xl bg-black"></div>
                            <div className="mt-10">
                                <div className="grid grid-cols-2 gap-14 gap-y-10">
                                    <p className="text-base leading-loose">
                                        Sura Vision - is a trailblazing investment firm that envisions a future of innovation and transformative impact. Our journey began with a passionate group of visionary investors who saw the immense potential in supporting startups that challenge the status quo and drive positive change in various industries.
                                    </p>
                                    <p className="text-base leading-loose">
                                        Sura Vision - is a trailblazing investment firm that envisions a future of innovation and transformative impact. Our journey began with a passionate group of visionary investors who saw the immense potential in supporting startups that challenge the status quo and drive positive change in various industries.
                                    </p>
                                    <p className="text-base leading-loose">
                                        Sura Vision - is a trailblazing investment firm that envisions a future of innovation and transformative impact. Our journey began with a passionate group of visionary investors who saw the immense potential in supporting startups that challenge the status quo and drive positive change in various industries.
                                    </p>
                                    <p className="text-base leading-loose">
                                        Sura Vision - is a trailblazing investment firm that envisions a future of innovation and transformative impact. Our journey began with a passionate group of visionary investors who saw the immense potential in supporting startups that challenge the status quo and drive positive change in various industries.
                                    </p>
                                </div>
                                <div className="flex justify-between items-center text-center mt-20 w-[70%] mx-auto">
                                    <div>
                                        <h3 className="font-bold mb-5 text-4xl">2025</h3>
                                        <p className="text-base">Founded In</p>
                                    </div>
                                    <div>
                                        <h3 className="font-bold mb-5 text-4xl">500+</h3>
                                        <p className="text-base">Farmers Supported</p>
                                    </div>
                                    <div>
                                        <h3 className="font-bold mb-5 text-4xl">2025</h3>
                                        <p className="text-base">Farmers Supported</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </>
            </DisplayLayout>
        </div>
    );
}
